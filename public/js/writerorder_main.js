...
function submitOrder() //提交订单
{
    ...
    $('.addressul').children('li').each(function(){
        ...
        if(rgb2hex($(this).css('border-color')) == "#009966") //选中的
        {
    ...
    window.location.href='OrderClServlet?dowhat=submitOrder&Aid=' + aid;
...
function readAddress(){
    ...
    $.ajax({
        type:"post",
        url:"AddressClServlet?dowhat=readAddress",
...
function deleteAddress(element,aid){
    ...
    {
        isChecked=false;
        if(rgb2hex($(element).parent('li').css('border-color')) == "#009966") //要删除的正好是选中的那个地址
            isChecked=true;
        ...
        $.ajax({
            type:"post",
            url:"AddressClServlet?dowhat=deleteAddress",
            data:{
                Aid:aid,
                isChecked:isChecked
            },
            success:function(){
                ...
                //还有其它地址栏且选中的那个要删
                if(isChecked == true)
                    $('.addressul').children('li').first().css('border-color','#009966'); //将第1个选上
            }
        });
    }
}
...
function addAddress(){
    ...
    else
    {
        ...
        $.ajax({
            type:"post",
            url:"AddressClServlet?dowhat=addAddress",
            data:{
                receiver_name:receiver_name,
                receive_address:receive_address,
                receive_code:receive_code,
...
function checkAddress(element,aid){
    ...
    if(rgb2hex($(element).parent('li').css('border-color')) != "#009966")
    {
        $.ajax({
            type:"post",
            url:"AddressClServlet?dowhat=checkAddress",
            data:{
                Aid:aid
            },
            success:function(){
                ...
                clearLastCheckBorder();
                $(element).parent('li').css('border-color','#009966');
...
function showAddressHTML(address){
    ...
    //是否用选中（框变绿色）
    if(address.Acheck == 1)
        $('.addressul').children('li').last().css('border-color','#009966');
}

//获得当前check与否
function getAddressCheck(element){

    if(rgb2hex($(element).parent('li').css('border-color')) == "#009966") //选中
        return 1;
    else
        return 0;
}
...
function clearLastCheckBorder(){
    ...
    $('.addressul').children('li').each(function(){
        ...
        if(rgb2hex($(this).css('border-color')) == "#009966")
        {
...
function sendBookBackToCart(element){
    ...
    {
        orderIndex=$(element).parents('.curorder_div').index()-1; //第几个订单
        ...
        $.ajax({
            type:"post",
            url:"OrderClServlet?dowhat=sendBookBackToCart",
            data:{
            ...
            success:function(data){
                ...
                if(orderlength == 1) //界面上直接删除订单
                {
                    ...
                    $(element).parents('.curorder_div').remove();
                    ...
                    if($('.curorder_div').length == 0) //全删除
                    {
                        $('.writeorder_info').append("<div class='emptyorder_div'><span class='font_bold_gray'>订单是空的哟~快去其它地方逛逛吧!</span><br><br>\
	                                   ...
	                                   <a href='CartClServlet?dowhat=queryCart' class='green_a'>返回购物车</a></div>");
